...
void AMatchingCube3D_Smooth_WorldSample::CF_Init()
{
	...
	Buffer_Normal.Reset();
	Buffer_Normal.Reserve(ChunkSize.X * ChunkSize.Y * ChunkSize.Z * 10);
...
void AMatchingCube3D_Smooth_WorldSample::CF_FillAndCreate()
{
	...
	for (int32 i = 0; i < ChunkSize.X; ++i)
	{
		for (int32 j = 0; j < ChunkSize.Y; ++j)
		{
			for (int32 k = 0; k < ChunkSize.Z; ++k)
			{
				...
				{
					// because the sphere dont has the lerp value ,so its error
					float dis_squad = FVector::DistSquared(FVector(i,j,k),Center);
					Map[GetIndex] = dis_squad <= Radius ? 1 : 0;
					Map_Value[GetIndex] = dis_squad;
				}
			}
		}
	}
	...
	ProceduralMeshComponent->CreateMeshSection_LinearColor(0,Buffer_Vertex,Buffer_Triangle,Buffer_Normal,{},{},{},true);
...
void AMatchingCube3D_Smooth_WorldSample::CF_CheckCubeAndFillData(int32 X, int32 Y, int32 Z)
{
	...
	{
		FVector p0 = RealPointList[EdgeList[i * 3 + 2]];
		FVector p1 = RealPointList[EdgeList[i * 3 + 1]];
		FVector p2 = RealPointList[EdgeList[i * 3]];
